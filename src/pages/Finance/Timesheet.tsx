...
const Timesheet: React.FC = () => {
  ...
  const handleDataFilter = useCallback(
    (newFilterData: Partial<FilterCollection>) => {
  ...
  const {
    ...
    isLoading,
    isValidating,
  } = useSWR([`get-pos`, city], () => getPoses({ placementId: city }), {
    revalidateOnFocus: false,
    revalidateOnReconnect: false,
    keepPreviousData: true,
  });
  ...
  const shifts = useMemo(() => {
    return (
      filter?.shiftReportsData.map((item) => ({
        ...item,
        posName: poses.find((pos) => pos.value === item.posId)?.name || "-",
        createdByName:
          workers.find((work) => work.value === item.createdById)?.name || "-",
      })) || []
    );
      ...
      {filterIsLoading ? (
        ...
        <div className="mt-8">
          <DynamicTable
            ...
            showPagination={true}
            navigableFields={[
              { key: "posName", getPath: () => "/finance/timesheet/creation" },
            ]}
          />
        </div>
      ) : (
