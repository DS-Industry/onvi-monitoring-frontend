...
import { Modal, Steps, Form, DatePicker, TimePicker, Select, Button } from 'antd';
        ...
        return (
          <div className="space-y-4">
            <div className="bg-blue-50 p-4 rounded-lg mb-4">
              <h4 className="font-medium text-blue-800 mb-2">{t('shift.shiftStart')}</h4>
              <div className="grid grid-cols-2 gap-4">
                <Form.Item
                  name="startDate"
                  label={t('shift.startDate')}
                  rules={[{ required: true, message: t('shift.startDateRequired') }]}
                >
                  <DatePicker
                    className="w-full"
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              <h4 className="font-medium text-green-800 mb-2">{t('shift.shiftEnd')}</h4>
              <div className="grid grid-cols-2 gap-4">
                <Form.Item
                  name="endDate"
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-1 text-sm text-gray-600">
                <p>{t('shift.startDateTime')}: {formData.startDate?.format('YYYY-MM-DD')} {formData.startTime?.format('HH:mm')}</p>
                <p>{t('shift.endDateTime')}: {formData.endDate?.format('YYYY-MM-DD')} {formData.endTime?.format('HH:mm')}</p>
                <p>{t('shift.user')}: {(employees || []).find(u => u.props.id === formData.userId)?.props.name} {(users || []).find(u => u.id === formData.userId)?.surname}</p>
              </div>
            </div>
          </div>
        );
