...
const CashCollectionDeviceTypeTable: React.FC<Props> = ({
    ...
    const getColumns = () => [
        ...
        },
        {
            ...
            render: (val: number) => <div className={"text-errorFill"}>
                {`${formatNumber(val)} ₽`}
            </div>,
        },
        {
    ...
    return (
        <Table
            ...
            summary={() => {
                const totalSumFact = tableData.reduce((acc, item) => acc + (item.sumFactDeviceType || 0), 0);
                const totalShortage = tableData.reduce((acc, item) => acc + (item.shortageDeviceType || 0), 0);
                const totalVirtual = tableData.reduce((acc, item) => acc + (item.virtualSumDeviceType || 0), 0);
                return (
                    <Table.Summary fixed>
                        <Table.Summary.Row>
                            ...
                            <Table.Summary.Cell index={3}><strong>{`${totalSumFact} ₽`}</strong></Table.Summary.Cell>
                            <Table.Summary.Cell index={4}><strong>{`${totalShortage} ₽`}</strong></Table.Summary.Cell>
                            <Table.Summary.Cell index={5}><strong>{`${totalVirtual} ₽`}</strong></Table.Summary.Cell>
                        </Table.Summary.Row>
                    </Table.Summary>
                );
            }}
