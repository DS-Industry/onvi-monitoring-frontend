...
import {getSaleDocuments, SALE_DOCUMENTS_RESPONSE} from "@/services/api/sale";
...
const SaleDocument: React.FC = () => {
    ...
    const filterParams = useMemo(
        ...
        [dateStart, dateEnd, warehouseId, currentPage, pageSize, formattedDate]
    );
    const [tableData, setTableData] = useState<SALE_DOCUMENTS_RESPONSE[]>([]);

    const swrKey = `get-all-sale-documents-${filterParams.warehouseId}-${filterParams.dateStart}-${filterParams.dateEnd}`;
    const { isLoading: saleDocumentLoading } = useSWR(
        swrKey,
        () => getSaleDocuments(filterParams),
        {
