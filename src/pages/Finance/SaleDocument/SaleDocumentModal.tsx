...
import {getAllStockLevelSales, GET_STOCK_LEVEL_SALE_RESPONSE} from "@/services/api/warehouse";
...
const SaleDocumentModal: React.FC<SaleDocumentModalProps> = ({
    ...
    const [availableNomenclatures, setAvailableNomenclatures] = useState<GET_STOCK_LEVEL_SALE_RESPONSE[]>([]);
    ...
    useEffect(() => {
        if (stockData) {
            const available = stockData.filter(item =>
                !existingNomenclatureIds.includes(item.nomenclatureId) && item.quantity > 0
            );
    ...
    const handleSubmit = () => {
        if (selectedNomenclatureData && count > 0 && count <= selectedNomenclatureData.quantity) {
    ...
    const canSubmit = selectedNomenclatureData &&
        count > 0 &&
                ...
                {selectedNomenclatureData && (
                    <>
                        ...
                        <div>
                            ...
                            <Input
                                value={(selectedNomenclatureData.price * count).toLocaleString('ru-RU')}
                                disabled
                                className="w-full"
                            />
                        </div>
