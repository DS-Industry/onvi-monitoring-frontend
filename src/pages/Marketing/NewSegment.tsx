...
const inputComponents: Record<string, React.ReactNode> = {
  ...
  registrationDate: (
    <Input type="date" title="Дата регистрации" classname="w-80" />
  ),
  visitCount: (
...
const NewSegment: React.FC = () => {
  ...
  const groupedInputs = Object.entries(groupKeys).reduce(
    (acc, [group, keys]) => {
      const groupSelectedColumns = keys.filter(key =>
