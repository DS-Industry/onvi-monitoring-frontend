...
type DataType = PaymentUpdateRequest &
  TablePayment & {
    key: string;
    id: number;
  };
...
const SalaryCalculation: React.FC = () => {
  ...
  const { trigger: updatePay, isMutating: updatingPayment } = useSWRMutation(
    ['update-payment'],
    async (
      _,
      {
        ...
        arg: PaymentUpdateRequest;
      }
    ) => {
      return updatePayment(arg);
    }
  );
  ...
  const payments = useMemo<DataType[]>(() => {
    return (
      data?.map(pay => ({
        ...pay,
        id: pay.id,
  ...
  const edit = (record: Partial<DataType> & { key: React.Key }) => {
    form.setFieldsValue({
      ...record,
      id: record.id,
      paymentId: record.paymentId, // ✅ include it
  ...
  const save = async (key: React.Key) => {
    try {
      ...
      if (index > -1) {
        ...
        const apiPayload = {
          paymentId: item.paymentId || item.id || row.id,
          prize: updatedItem.prize,
          fine: updatedItem.fine,
          virtualSum: updatedItem.virtualSum,
          comment: updatedItem.comment,
          payoutTimestamp: dayjs(updatedItem.payoutTimestamp).toDate(),
        };

        const result = await updatePay(apiPayload);
        if (result) {
          mutate([
            'get-payments',
            startPaymentDate,
      ...
      async onOk() {
        try {
          const result = await mutate(
            [`delete-manager-data`],
            () =>
              deletePayments({ ids: selectedRowKeys.map(key => Number(key)) }),
            false
          );
  ...
  const columnsSalaryCalculation = [
    ...
    },
    {
      ...
      sorter: (a: DataType, b: DataType) =>
        a.totalPaymentFinal - b.totalPaymentFinal,
      render: currencyRender,
      editable: false,
    },
    {
  ...
  const mergedColumns: TableProps<DataType>['columns'] =
    columnsSalaryCalculation.map(col => {
      if (!col.editable) {
        return col;
      }
      return {
