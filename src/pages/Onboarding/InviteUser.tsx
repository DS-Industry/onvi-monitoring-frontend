...
const InviteUser: React.FC = () => {
  ...
  const { trigger, isMutating } = useSWRMutation(['create-worker'], async () =>
    createUserRole(
      {
  ...
  useEffect(() => {
    if (!validUser || !validUser.status || validUser.status !== 'SUCCESS') {
      setIsError(true);
    } else {
      setIsError(false);
    }
    ...
    <div>
      {isError ? (
        <div className="flex flex-col justify-center items-center">
          <NoDataUI title={t('organizations.user')} description={''}>
            <img src={NoToken} className="mx-auto" loading="lazy" alt="Token" />
          </NoDataUI>
        </div>
      ) : (
        <div className="flex min-h-screen items-center justify-center bg-background02 p-4">
          <div className="flex flex-col rounded-lg p-8 lg:flex-row md:p-0">
            <div className="lg:w-5/12 p-8 lg:ml-40">
              ...
              <p className="text-3xl font-extrabold leading-[1.25] text-text01 mt-20 mb-1">
                {t('forgot.set')}
              </p>
              <p className="font-normal text-text01 text-base mb-5">
                {t('forgot.please')}
              </p>
              <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex items-center justify-between mb-10">
                  <label className="inline-flex items-center">
                    <input
                      type="checkbox"
                      className="rounded text-primary02 border-text02"
                    />
