...
const Pos: React.FC = () => {
  ...
  const placements: { country: string; city: string; value: number }[] =
  ...
  const organization: { name: string; value: number }[] =
  ...
  const organizationIds = Array.from(
    new Set(data?.map(item => item.organizationId))
  );
  ...
  const poses: Pos[] =
    data
      ?.map((item: Pos) => ({
        ...
        createdByName:
          `${createContacts?.[item.createdById]?.name || ''} ${createContacts?.[item.createdById]?.surname || ''}` ||
          '-',
        updatedByName:
          `${updateContacts?.[item.updatedById]?.name || ''} ${updateContacts?.[item.updatedById]?.surname || ''}` ||
          '-',
        city:
          placements.find(place => place.value === item.placementId)?.city ||
          '',
        country:
