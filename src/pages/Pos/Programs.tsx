...
const Programs: React.FC = () => {
    ...
    const portalPrograms = programRaw?.filter((item) => item.posType === "Portal") || [];

    const childColumns: ColumnsType<ProgramDetail> = portalPrograms.length > 0
        ? [
            { title: "Программа", dataIndex: "programName", key: "programName" },
            {
                title: "Кол-во программ", dataIndex: "counter", key: "counter", render: (_value, record) => formatNumber(record.counter),
            },
            { title: "Общее время (мин)", dataIndex: "totalTime", key: "totalTime", render: (_value, record) => formatNumber(record.totalTime) },
            { title: "Среднее время (мин)", dataIndex: "averageTime", key: "averageTime" },
            { title: "Выручка", dataIndex: "totalProfit", key: "totalProfit", render: currencyRender },
            { title: "Средний чек", dataIndex: "averageProfit", key: "averageProfit", render: currencyRender },
        ]
        : [
            { title: "Программа", dataIndex: "programName", key: "programName" },
            { title: "Кол-во программ", dataIndex: "counter", key: "counter", render: (_value, record) => formatNumber(record.counter) },
    ...
    const aggregateProgramsData = (programs: ProgramDevice[]) => {
        const map = new Map<string, { programName: string; counter: number; totalProfit: number }>();
                                    ...
                                    datasets: [
                                        {
                                            ...
                                            backgroundColor: barData.map((_item, i) => getRandomColor(i)),
                                        },
                                    ],
                                }}
                                options={{
