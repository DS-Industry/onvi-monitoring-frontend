...
import { Card, Row, Col, Typography, List, Checkbox, Radio, RadioChangeEvent } from "antd";
...
const TariffCard: React.FC<TariffType> = ({
    ...
    return (
        <Card
            className="mb-6 bg-background05 rounded-lg"
            variant="borderless"
        >
            <Row className="flex items-center space-x-4">
                <Col>
                    <Title level={4} style={{ color: "#0B68E1" }} className="!mb-0 text-blue-600">
                        {name}
                    </Title>
                </Col>
                <Col>
            ...
            <Row gutter={[0, 8]} className="mt-8">
                ...
                <Col span={16}>
                    <div className="flex items-end space-x-1 w-full">
                        <Text className="whitespace-nowrap font-semibold text-lg">{t("subscriptions.in")}</Text>
                ...
                <Col span={16}>
                    <div className="flex items-end space-x-1 w-full">
                        <Text className="whitespace-nowrap font-semibold text-lg">{t("subscriptions.for")}</Text>
                        <div className="flex-grow border-b border-gray-300" />
    ...
    return (
        <>
            <div className="max-w-3xl py-6 px-4">
                {tariffs.map((tariff) => (
                    <TariffCard key={tariff.name} {...tariff} t={t} setButtonOn={setButtonOn} />
                ))}
            </div>
            <DrawerCreate>
                <form className="space-y-6">
                    <div className="font-semibold text-2xl text-text01">{t("subscriptions.payment")}</div>
                    <div className="font-semibold text-lg text-primary02">{t("subscriptions.opt")}</div>
                    <div className="grid grid-cols-[200px_1fr] gap-y-6 gap-x-10">
                        <div className="text-text01">{t("subscriptions.method")}</div>
                        <Radio.Group onChange={onChange} value={paymentType} className="space-y-2 flex flex-col">
                            <Radio value="bank" className="!flex !items-start">
                                <div>
                                    <div className="text-base text-text01">{t("subscriptions.bank")}</div>
                                    <div className="text-sm text-text02">{t("subscriptions.invoice")}</div>
                                </div>
                            </Radio>
                            <Radio value="online" className="!flex !items-start">
                                <div>
                                    <div className="text-base text-text01">{t("subscriptions.online")}</div>
                                    <div className="text-sm text-text02">{t("subscriptions.master")}</div>
                                </div>
                            </Radio>
                        </Radio.Group>
                        ...
                        <div className="font-semibold text-lg text-text01">с 3 августа 2024 по 3 октября 2024</div>

                        <div className="text-text01 text-2xl font-semibold">{t("finance.total")}:</div>
