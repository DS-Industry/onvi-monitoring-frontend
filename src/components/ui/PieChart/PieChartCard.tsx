...
const PieChartCard: React.FC = () => {
    ...
    const pieData1 = {
        labels: ["Батайск, Урванцева", "Батайск, СЖМ", "Ростов-на-Дону, Жданова", "Ростов-на-Дону, Ленина"],
        datasets: [
            {
                data: [3, 23, 15, 59],
    ...
    return (
        <div className="rounded-2xl shadow-card p-6 bg-white w-[1128px]">
            <div className="text-sm uppercase text-text02 mb-4 w-40">{t("marketing.cust")}</div>
                        ...
                        <div className="flex-1 flex items-center justify-center">
                            <ul className="space-y-1">
                                {pieData1.labels.map((label, index) => (
                                    <li key={index} className="flex items-center gap-2 text-nowrap">
                                        <span
                                            className="w-4 h-4 rounded-full"
                                            style={{ backgroundColor: pieData1.datasets[0].backgroundColor[index] }}
                                        ></span>
