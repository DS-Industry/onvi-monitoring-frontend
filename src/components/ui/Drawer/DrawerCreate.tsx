...
const DrawerCreate: React.FC<DrawerCreateProps> = ({
  children,
  classname,
  onClose,
}: DrawerCreateProps) => {
    ...
    <div className="drawer drawer-end w-full">
      ...
      <div className="drawer-side z-[1050] fixed top-0 right-0 h-full w-full">
        <label
          ...
          className="drawer-overlay bg-black/50 z-[1040] fixed inset-0"
        />
        <ul
          className={`menu bg-background02 text-base-content min-h-full w-full sm:w-3/4 md:w-1/2 ${classname} p-4 z-[1051]`}
        >
          {children}
        </ul>
      </div>
