...
const ExpandableTable = <T extends TableRow>({
  ...
  const getStatusTag = (status: string) => {
    ...
    if (
      status === t('tables.SAVED') ||
      status === t('tables.VERIFICATE') ||
      status === t('tables.SAVE')
    )
    ...
    const baseColumn: EnhancedTableColumn<T> = {
      ...
      render: (value: unknown, record: T): React.ReactNode => {
        ...
        return navigableField ? (
          ...
          (col.render(record, handleChange) as React.ReactNode)
        ) : (
  ...
  const expandedRowRender = (record: TableRow) => {
    const filteredData =
      dataSource &&
      dataSource.filter(item => item.deviceId === record.deviceId);
