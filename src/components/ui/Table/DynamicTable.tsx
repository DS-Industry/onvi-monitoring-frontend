...
interface EnhancedTableColumn<T extends TableRow = TableRow>
...
const DynamicTable = <T extends TableRow>({
  ...
  const enhancedColumns: EnhancedTableColumn<T>[] = displayedColumns.map(
    col => {
        ...
        render: (value: unknown, record: T) => {
          ...
          if (col.type === 'date') {
            ...
            if (value === null || value === undefined) {
              ...
              date = TableUtils.createDateTimeWithoutComma(
          ...
          return navigableField ? (
            ...
            col.render(record, handleChange)
          ) : value ? (
            String(value)
          ) : (
            '-'
  ...
  const allowed = hasPermission(
    userPermissions,
    getRequiredPermissions(activePage?.path || '')
  );
  ...
  return (
    <div>
      {tableTitle && (
        <h2 className="text-lg font-semibold mb-4">{tableTitle}</h2>
      )}
      {title && (
        <span
