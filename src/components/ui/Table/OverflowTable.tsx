        ...
        <div className="max-w-full overflow-x-auto">
          {title && (
            <span
              ...
              onClick={() =>
                navigate(`${nameUrlTitle}`, { state: { ownerId: urlTitleId } })
              }
            >
              ...
              <tr>
                ...
                >
                  {allowed =>
                    allowed &&
                    isUpdateLeft && (
                      <th className="border border-background02 bg-background06 w-11 min-w-11"></th>
                    )
                  }
                </Can>
                {columns.map(
              ...
              {currentData?.map((row: TableRow) => (
                <tr key={row.id}>
                  ...
                  {displayedColumns.map(column => (
                    <td
                      key={column.key}
                      className="border-b border-x-4 border-b-[#E4E5E7] border-x-background02 bg-background02 py-2 px-2.5 text-start whitespace-nowrap text-sm text-text01 overflow-hidden overflow-x-visible"
                    >
                      {(column.key === 'name' ||
                        (column.key === 'posName' && !row.name)) &&
                      nameUrl ? (
                        <span
                          ...
                          onClick={() => {
                            navigate(`${nameUrl}`, {
                              state: {
                                ...
                                status: row.status,
                                type: row.type,
                                workDate: row.startWorkDate,
                                endDate: row.endSpecifiedDate,
                              },
                            });
                            setDocumentType(
                              documentTypes.find(doc => doc.name === row.type)
                                ?.value || ''
                            );
                          }}
                        >
                        ...
                        row[column.key] ? (
                          <div
                            className={`${Number(row[column.key]) < 0 || (column.key === 'shortageDeviceType' && Number(row[column.key]) > 0) ? 'text-errorFill' : ''}`}
                          >
                            {formatNumber(Number(row[column.key]), column.type)}
                          </div>
                        ) : (
                          '-'
                        )
                        ...
                        row[column.key] ? (
                          <div
                            className={`${Number(row[column.key]) < 0 ? 'text-errorFill' : ''}`}
                          >{`${formatNumber(Number(row[column.key]))}%`}</div>
                        ) : (
                          '-'
                        )
                      ) : column.render ? (
                        column.render(row, handleChange)
                      ) : renderCell ? (
                        renderCell(column, row)
                      ) : column.key.toLocaleLowerCase().includes('status') ? (
                        <div
                          className={`flex items-center justify-center gap-2 whitespace-nowrap text-ellipsis overflow-hidden
                             ${
                               row[column.key] === t('tables.ACTIVE') ||
                               row[column.key] === t('tables.SENT') ||
                               row[column.key] === t('analysis.DONE')
                                 ? 'text-[#00A355]'
                                 : row[column.key] === t('tables.OVERDUE') ||
                                     row[column.key] === t('analysis.ERROR')
                                   ? 'text-errorFill'
                        ...
                        <div className="whitespace-nowrap text-ellipsis overflow-hidden">
                          <div className="whitespace-nowrap text-ellipsis overflow-hidden">
                            {column.type === 'date'
                              ? row[column.key]
                                ? TableUtils.createDateTimeWithoutComma(
                                    row[column.key] as Date | string,
                                    userTimezone
                                  )
                                : '-'
                              : column.type === 'period'
                                ? row[column.key]
                                  ? formatPeriodType(String(row[column.key]))
                                  : '-'
                  ...
                  {displayedColumns.slice(3).map(column => (
                    <td key={column.key} className="p-2 text-right">
                      {column.type === 'number'
                        ? formatNumber(
                            currentData.reduce(
                              (sum: number, row: { [x: string]: unknown }) =>
