...
const SegmentsDialog: React.FC<Props> = ({
    ...
    const columnGroups: { [key: string]: TableColumn[] } = {
        "СЕГМЕНТАЦИЯ ПО ДАННЫМ КЛИЕНТА": filteredColumns.filter(
            (col) => col.key === "clientType" || col.key === "tags" || col.key === "registrationDate"
        ),
                    ...
                    <div className="bg-white w-full max-w-4xl rounded-lg p-6 relative">
                        ...
                        <div className="flex">
                            <div className="w-3/12 pr-4 space-y-2">
                                {selectedColumns.map((key) => {
                                    ...
                                    return (
                                        column && (<div className="text-base text-text01">
                                            {column.label}
                                        </div>)
                                    )
                                })}
                        ...
                        <div className="flex justify-between mt-6">
                            <Button
                                ...
                                handleClick={() => { setIsModalOpen(false); setSelectedColumns(selectedColumns);}}
                            />
                            <Button
                                title={t("warehouse.reset")}
                                handleClick={() => { setIsModalOpen(false); setSelectedColumns([]); }}
                                type="outline"
                            />
                        </div>
                    </div>
