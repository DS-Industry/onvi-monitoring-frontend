...
- Норму расхода ставит главный техник (коэффициенты объёма химии от времени работы).
  ...
  Информационный характер + график расхода/залива/слива. Выводы делает техник.
...
- **Chemical report** — не отдельная таблица: GET chemistry-report по FINISHED-задачам с `code=TECH_RATE` и `ProgramTechRate`. В `chemistryAddLevels` поля `level`, `add`, `drain` (слив), `techTaskDate`.
