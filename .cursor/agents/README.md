...
63 specialized AI agents for the onviteam-cursor-handbook project. Each agent focuses on a specific domain and can be invoked via `/agent-name` or `@agent-name`.
...
| Agent | Purpose |
|---|---|
| `mobile-ui-agent` | Expo/RN screens, Gluestack, NativeWind, Expo Router |
| `accessibility-agent` | WCAG / mobile accessibility audit (shared) |
...
| `multi-role-agent` | Cross-domain task coordination; `/kickoff` and `/execute` team lead |
